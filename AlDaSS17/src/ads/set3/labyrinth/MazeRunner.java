...
public class MazeRunner {
	...
	public static List<Direction> findPath(boolean[][] labyrinth, int startX, int startY, int endX, int endY) {
		...
		while (currCell.getX() != endX && currCell.getY() != endY) {
			closedCells.add(currCell);
			
			for (int i = 0; i < dirs.length; i++) {
				
				int newX = translateX(currCell.getX(), dirs[i]);
				int newY = translateY(currCell.getY(), dirs[i]);	
		...
		while (currCell.getX() != startX && currCell.getY() != startY) {
			path.addFirst(currCell.getDir());
			int prevX = translateX(currCell.getX(), invert(currCell.getDir()));
			int prevY = translateY(currCell.getY(), invert(currCell.getDir()));	
...
class Cell implements Comparable<Cell> {
	...
	public Cell(int x, int y, int targetX, int targetY) {
		this.x = x;
		this.y = y;
